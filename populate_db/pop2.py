...
class Tournament:
    def __init__(self, tournament_id, name, start_date, end_date, location):
...
sport_rules = {
    ...
    "volleyball": "Volleyball is played by two teams of six players each on a rectangular court divided by a net. Players aim to score points by hitting a ball over the net and landing it in the opponent's court. Matches are played in sets, and the team that wins the majority of sets wins the match."
}

sports_team_count_in_match = [2, 2, 2, 2, 16]

# Load data and initialize variables
current_pwd = os.getcwd()
names = pd.read_csv(current_pwd+"\\name_surname.csv")
sports_names = ["baseball", "tennis", "basketball", "soccer","pubg E-tournament", "rugby", "cricket", "golf", "voleyball"]
...
for i in range(num_tournaments):
    ...
    tournament = Tournament(
        ...
        end_date=end_date,
        location=tournament_location_dict[rand.randint(0, len(tournament_location_dict) - 1)]
    )
    tournament_objs.append(tournament)

for tournament in tournament_objs:
    mycursor.execute(
        "INSERT INTO Tournaments (tournament_id, name, start_date, end_date, location) VALUES (%s, %s, %s, %s, %s)",
        (tournament.tournament_id, tournament.name, tournament.start_date, tournament.end_date, tournament.location)
    )
mydb.commit()
...
for i, sport in enumerate(sports_names):
    sports_rows.append({
        ...
        "team_count_in_match": sports_team_count_in_match[i],
        "tournament_id": rand.randint(0, num_tournaments - 1)
    })
# Insert sports data
for row in sports_rows:
    mycursor.execute(
        "INSERT INTO Sports (sport_id, name, description, rules, team_count_in_match, tournament_id) VALUES (%s, %s, %s, %s, %s, %s)",
        (row["sport_id"], row["name"], row["desc"], json.dumps(row["rules"]), row["team_count_in_match"], row["tournament_id"])
    )
mydb.commit()
